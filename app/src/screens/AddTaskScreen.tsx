...
// Layout follows the natural sentence: what -> how long -> who -> when.
// Happy path stays 3 taps (category, duration, save); backdating covers the last 7 days.
// Same screen handles edit mode: pre-filled from the task, save updates instead of creating.
export default function AddTaskScreen({ onDone, editTask }: {
  ...
  // last 7 days as chips: Today, Yesterday, then "Mon 2" style
  ...
  const occurredAt = useMemo(() => {
    // keep the original clock time when editing; new tasks use the current time
  ...
  const save = () => {
    ...
    if (isEdit && editTask) {
      updateTask(editTask.id, { categoryKey, title: title || undefined, durationMin, memberId, occurredAt });
      onDone(copy.addTask.savedEdit);
      return;
    }
    const firstEver = state.tasks.length === 0;
    addTask({ categoryKey, title: title || undefined, durationMin, memberId, occurredAt });
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={{ flexDirection: 'row', marginTop: spacing.s }}>
          {state.members.map((m) => (
            ...
            >
              <Avatar name={m.name} colour={m.colour} size={48} selected={memberId === m.id} />
        ...
        {!state.hideMoney && categoryKey && (
          <Text style={[type.caption, styles.section]}>
            {copy.addTask.valuePreview(currencySymbol[state.currency] ?? '', value)}
            {'  ·  '}{copy.addTask.valuePreviewSub}
          </Text>
        )}
