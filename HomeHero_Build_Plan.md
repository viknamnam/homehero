...
- [~] Set up repo, CI/CD, environments (repo + GitHub + staging Supabase + **CI live: typecheck, copy-lint, preflight on every push** — prod env pending)
...
- [~] Crash reporting wired (Sentry integrated, errors-only, PII off — flip to done when DSN is set in .env)
- [~] Alpha distribution (EAS configured: dev/preview/production profiles, app identifiers set — first build pending `eas init` + `eas build`)
...
- [ ] 🔒 Family Balance screen behind feature flag:
  ...
  - [x] Appreciation summary appears **above or beside** percentages (Home Value screen built, flag-off)
  - [x] Invaluable framing on mental-load value (built into Home Value value-note, flag-off)
...
| 22 | 05 Jun 2026 | SDK-downgrade incident: assistant's environment carried a stale SDK-51 manifest after the upgrade was applied only on the local machine; a routine zip silently downgraded the project | Root cause: dual sources of truth + blind-overwrite delivery. Fixed: Section 12.5 protocol (repo is truth, mirror confirmation, locked-stack line, preflight script, install fingerprints, git-diff review gate) + `scripts/preflight.js` added and run before every zip | all | Resolved |
| 23 | 05 Jun 2026 | Expo Go tether fragility: tunnel drops when the app backgrounds (camera/photo picker), repeated "cannot connect to Expo CLI" | Adopted **EAS development builds** pre-alpha: installable APK, no Expo Go, stable connections; also the path to TestFlight/Play alpha anyway. eas.json profiles + app identifiers committed | 1 | Decided |
| 24 | 06 Jun 2026 | Phase 2 screens (Thanks + Home Value) built behind FLAGS (thanks/homeValue/planTheDay = false). Home Value = view over existing data (appreciation-first, teamwork framing, no "lowest contributor", gentle rebalancing). Thanks = pick person + optional category/note, weekly wins. Thanks entity is local-first | **Before flipping flags:** (1) ~~cloud sync for thanks~~ DONE (migration 0005: appreciations + client_id/category_id, push/pull wired), (2) household testing validates framing, (3) Thanks + Home Value flip TOGETHER. Plan-the-Day CTA present but tools deferred | 2 | In progress |
| 25 | 06 Jun 2026 | Trust layer shipped: Settings "Your data" card — export household JSON (share sheet, clipboard fallback) + delete household everywhere (cloud cascade via 0005 hh_delete policy, then local reset + sign-out) | **Alpha scope decision:** any linked member can delete the household (policy is household-scoped, not role-gated). Role-gated deletion arrives with roles/permissions in Phase 3. Two-tap confirm in UI | 2, 3 | Decided |
| 26 | | | | | |

---

## 12.5 Delivery & environment protocol (added after the SDK-downgrade incident — see Gaps #22)

**Single source of truth: the GitHub repo.** The assistant's build environment is a *mirror* of the repo, never an authority over it. Two standing rules:

1. **No side-channel changes.** Any change applied directly on the local machine (pasted file contents, dashboard settings with code impact, dependency bumps) must be mirrored into the assistant's environment *in the same session*, and confirmed ("mirrored ✔") before the next zip is produced.
2. **Locked environment line.** The current locked stack is recorded here and only changed deliberately:
   > **Expo SDK 56** · expo ^56.0.8 · react 19.2.3 · react-native 0.85.3 · TS ~6.0.3 · entry `index.ts`

**Assistant pre-zip checklist (every drop):**
- [ ] `scripts/preflight.js` passes (machine-checks the locked line above)
- [ ] Typecheck against the SDK-matched test environment
- [ ] Copy-lint passes
- [ ] Delivery note states the **expected install fingerprint** when package.json changed (e.g. "~520 packages, ~10 vulnerabilities")

**Local apply checklist (every drop):**
- [ ] Commit current state BEFORE copying any zip
- [ ] Copy zip over project, then **review `git diff` before doing anything else** — `package.json` and `app.json` diffs get extra scrutiny: versions must only move *forward*
- [ ] Only run `npm install` if `package.json` actually changed
- [ ] Compare the install output against the stated fingerprint; a vulnerability count that *jumps up* = stop and report, don't proceed
